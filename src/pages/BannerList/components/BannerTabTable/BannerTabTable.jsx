...
export default class BannerTabTable extends Component {
  ...
  componentWillMount(){
    const that=this;
    const result = ajaxTo('api.php?entry=sys&c=app&a=appbanner&do=display');
...
componentDidMount(){
  ...
  deleteId = (id) => {
    console.log(id);
    ajaxTo('https://app.yongketong.cn/api.php?entry=sys&c=app&a=appbanner&do=delete',{'bannerid':id});
  ...
  render() {
    ...
    if(forData){
      for (var i = 0; i < forData.length; i++) {
        ...
        arr.push({
          'bannername':forData[i].bannername,
          'publishTime':forData[i].createtime,
          'id':forData[i].id,
          'bannerstatus':forData[i].bannerstatus=='1'?'开启':'关闭',
