...
const tabs = [
  { tab: "banner列表", key: 0, content: "/homeslidelist"},
  { tab: "新建banner", key: 1, content: "/homeslide/create"},
];
...
export default class ComplexTabTable extends Component {
  ...
  componentWillMount(){
    const that=this;
    const result = ajaxTo('api.php?entry=sys&c=banner&a=bannerManage&do=bannerList');
...
that.setState({allData: res.data,pageNum:res.totle,currentPageNum:res.psize});
  ...
  deleteId = (id) => {
    console.log(id);
    ajaxTo('api.php?entry=sys&c=banner&a=bannerManage&do=delete',{'bid':id});
  ...
  renderOperations = (value, index, record) => {
    const toUrl = '/homeslide/'+record.id;
    return (
  ...
  changePage = (currentPage) => {
    ...
    ajaxTo('api.php?entry=sys&c=banner&a=bannerManage&do=bannerList',{page:currentPage})
    .then(function(res){
      console.log(res)
      that.setState({
  ...
  render() {
    ...
    if(forData){
      for (var i = 0; i < forData.length; i++) {
        ...
        arr.push({
          'title':forData[i].bannername,
          'sort':forData[i].displayorder,
          'link':forData[i].link,
          'id':forData[i].id,
          'status':forData[i].enabled=='1'?'开启':'关闭',
          'icon':forData[i].thumb,
          'pcicon':forData[i].webThumb
        })
      }
    }
    const tableData = {
