...
const {Row, Col} = Grid;
const tabs = [
  { tab: "课程章节列表", key: 0, content: "/chaptermanagelist"},
  { tab: "新建课程章节", key: 1, content: "/chaptermanage/create"},
];
...
export default class ComplexTabTable extends Component {
  ...
  componentWillMount() {
    const that = this;
    const result = ajaxTo('api.php?entry=sys&c=chapter&a=chapter&do=display');
  ...
  deleteId = (id) => {
    console.log(id);
    ajaxTo('api.php?entry=sys&c=chapter&a=chapter&do=del', {'cid': id});
  ...
  changePage = (currentPage) => {
    ...
    ajaxTo('api.php?entry=sys&c=chapter&a=chapter&do=display',{page:currentPage})
    .then(function(res){
      console.log(res)
      that.setState({
    ...
    if (forData) {
      for (var i = 0; i < forData.length; i++) {
        ...
        arr.push({
          ...
          'id': forData[i].id,
          'course':forData[i].tid.topic_name,
        ...
        <IcePanel style={{
          ...
          <IcePanel.Body>
            <Table dataSource={tableData.data} isLoading={tableData.__loading} className="basic-table" style={styles.basicTable} hasBorder={false} onRowClick={onRowClick}>
              <Table.Column title="文章标题" width={220} dataIndex="title"/>
              <Table.Column title="所属课程" width={100} dataIndex="course"/>
              ...
              <Table.Column title="排序" dataIndex="displayorder" width={150}/>
