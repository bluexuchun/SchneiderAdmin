...
const tabs = [
  {
    tab: "首页banner列表",
    key: 0,
    content: "/homeslideList"
  }, {
    tab: "新建首页banner",
    key: 1,
    content: "/homeslide/create"
  }
];
...
export default class CreateHomeSlideForm extends Component {
  static displayName = 'CreateHomeSlideForm';
  ...
  componentDidMount(){
    ...
    if(activityId != 'create'){
      ajaxTo('api.php?entry=sys&c=banner&a=bannerManage&do=edit',{'bid':activityId})
      .then((res)=>{
        console.log(res)
        ...
        this.setState({
          value:{
            title:currentData.bannername,
            sort:currentData.displayorder,
            link:currentData.link,
            pclink:currentData.pclink,
            status:currentData.enabled=='1'?true:false,
            appid:currentData.id,
          },
          logoImg:currentData.thumb,
          logoImg1:currentData.webThumb
  ...
  onSuccess1 = (res, file) => {
    console.log(res)
    Feedback.toast.success('上传成功');
    const logoImg1 = 'https://' + res.imgURL;
  ...
  submit = () => {
    ...
    that.formRef.validateAll((error, value) => {
      ...
      const dataAry = {
        ...
        webThumb: that.state.logoImg1,
        bid: this.props.history.params.id
      }
      console.log(that.formRef.props.value);

      //修改区
      const newrequestUrl = this.props.history.params.id == 'create'
        ? 'api.php?entry=sys&c=banner&a=bannerManage&do=update'
        : 'api.php?entry=sys&c=banner&a=bannerManage&do=update';
      ...
      result.then(function(res) {
        ...
        setTimeout(function() {
          that.props.history.router.push('/homeslidelist');
        }, 1000);
