...
export default class ComplexTabTable extends Component {
  ...
  componentWillMount(){
    const that=this;
    const result = ajaxTo('api.php?entry=sys&c=member&a=list');
...
that.setState({allData: res.data,pageNum:res.total,currentPageNum:res.psize});
  ...
  deleteId = (id) => {
    console.log(id);
    ajaxTo('api.php?entry=sys&c=member&a=list&do=delete',{'id':id});
  ...
  renderOperations = (value, index, record) => {
    const toUrl = '/authoritysetting/'+record.id;
    return (
  ...
  changePage = (currentPage) => {
    ...
    ajaxTo('api.php?entry=sys&c=member&a=list',{page:currentPage})
    .then(function(res){
      console.log(res)
      that.setState({
