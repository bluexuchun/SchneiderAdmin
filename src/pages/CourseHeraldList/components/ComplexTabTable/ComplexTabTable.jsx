...
export default class ComplexTabTable extends Component {
  ...
  componentWillMount(){
    const that=this;
    const result = ajaxTo('api.php?entry=sys&c=foreshow&a=foreshow&op=display');
  ...
  deleteId = (id) => {
    console.log(id);
    ajaxTo('api.php?entry=sys&c=foreshow&a=foreshow&op=del',{'fid':id});
  ...
  renderOperations = (value, index, record) => {
    const toUrl = '/courseherald/'+record.id;
    return (
  ...
  changePage = (currentPage) => {
    ...
    ajaxTo('api.php?entry=sys&c=foreshow&a=foreshow&op=display',{page:currentPage})
    .then(function(res){
      console.log(res)
      that.setState({
    ...
    if(forData){
      for (var i = 0; i < forData.length; i++) {
        ...
        arr.push({
          ...
          'author':forData[i].author,
          'startdate':forData[i].startdate,
          'id':forData[i].id,
