...
const tabs = [
  {
    tab: "系列课程列表",
    key: 0,
    content: "/coursemanagelist"
  }, {
    tab: "新建系列课程",
    key: 1,
    content: "/coursemanage/create"
  }
];
...
export default class ComplexTabTable extends Component {
  ...
  componentWillMount() {
    const that = this;
    const result = ajaxTo('api.php?entry=sys&c=app&a=regulation&do=display');
    ...
    if (forData) {
      for (var i = 0; i < forData.length; i++) {
        ...
        arr.push({
          ...
          'appid': forData[i].appid,
          'publishTime': forData[i].createtime,
          'id': forData[i].id,
          'publishStatus': forData[i].appstatus == '1'
            ? '开启'
            : '关闭',
        ...
        <IcePanel style={{
          ...
          <IcePanel.Body>
            <Table dataSource={tableData.data} isLoading={tableData.__loading} className="basic-table" style={styles.basicTable} hasBorder={false} onRowClick={onRowClick}>
              <Table.Column title="标题" width={220} dataIndex="appname"/>
              <Table.Column title="AppID" width={100} dataIndex="appid"/>
